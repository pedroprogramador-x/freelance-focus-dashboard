...
| **E2** | Backend skeleton + SQLite + **Safety Kernel + Path Runtime** | Processo local que sobe, responde, persiste e **já sabe validar path corretamente** | `api/` com FastAPI, `config`, `db`, Alembic, 7 tabelas, `GET /api/health`, `LocalSessionToken` injetado no HTML **pela FastAPI no build compilado**, `safety/` puro, `path_runtime.py`, CI da API | `/api/health` responde e é **a única rota sem token**; o token não entra no bundle, `.env` ou log; `alembic upgrade head` cria o schema com `Run.invocation_id` UNIQUE e **sem** a constraint antiga; suíte cobre `..`, symlink, junction, UNC, device namespace, drive-relative, alias 8.3 e **revalidação pós-abertura**; `npm test` do web **inalterado** | E1 | Ambiente Python no Windows |
...
| **E11** | UI completa | Fechar o laço para o usuário | SSE via `fetch`, diff, findings por `purpose` e vigência, runs com proveniência, prévia de purga de workspace/task, descarte de worktree; **e a entrega diferida da parte do [ADR-0001](../adr/0001-local-backend-fastapi-sqlite.md) item 7 relativa ao Vite dev (ver addendum do ADR-0001)** — os 6 itens: (1) launcher local do Vite; (2) canal privado **em memória** que transporta o `LocalSessionToken` da FastAPI para o Vite (sem prefixo `VITE_`, sem `.env`); (3) `transformIndexHtml` injetando a `<meta name="ff-session-token">` no modo dev, com `Cache-Control: no-store`; (4) garantias de que o token **não** entra em bundle, `.env` ou log; (5) o Vite dev em loopback, com allowlist de `Host` e CORS negado, **reaproveitando o guarda de `Origin`/`Host` da E3** (`app/api/security.py::same_origin_write_allowed`, `origin_matches_host`); (6) **gate automatizado** que sobe FastAPI + Vite dev de verdade e prova o fluxo ponta a ponta (dev server + proxy `/api` + token lido da `<meta>` + rota autenticada), não apenas que o código existe | Fluxo completo pela interface, sem terminal; nenhum dado não sanitizado no browser; **os 6 itens acima entregues e cada um coberto por teste** — em especial o item (6): um gate de CI que exercita o `npm run dev` real contra a FastAPI, distinto do gate de build+serve da E3 (`test_web_spa_integration.py`) | E10 | Vazamento na serialização |
...
> **Correção (auditoria independente da E3 — E3-AUD3-002).** A linha da E2 dizia que o
> `transformIndexHtml` por canal somente em memória no Vite dev havia sido entregue.
> **Foi entregue apenas** a injeção do `LocalSessionToken` **pela FastAPI no build
> compilado** — essa parte foi implementada e auditada na E2. A parte do
> [ADR-0001](../adr/0001-local-backend-fastapi-sqlite.md) item 7 referente ao
> **launcher / canal privado do Vite dev** foi identificada nesta auditoria como **não
> implementada** e formalmente **diferida para a E11** (ver o addendum do ADR-0001 e o
> Entregável/Gate da E11 acima, que lista os 6 itens). A E2 **permanece concluída e
> auditada** — apenas esta descrição textual foi corrigida; nenhuma implementação da E2
> foi reaberta.
...
| **Forma exata da confirmação de purga** (`confirm_phrase` ou token curto) | Ao implementar E3; o contrato já exige que **não** seja parâmetro de query. *Decidido em E3: token de curta duração em memória (`PurgeTokenStore`, TTL 60s, uso único).* |
| **Launcher de hot-reload** (FastAPI + Vite juntos, `LocalSessionToken` compartilhado em memória, `transformIndexHtml` no dev server) | **E11** — é a **entrega diferida** da parte do [ADR-0001](../adr/0001-local-backend-fastapi-sqlite.md) item 7 referente ao Vite dev: ver o **addendum do ADR-0001** (que registra a ausência, quando foi detectada e o reagendamento) **e a linha da E11** (que lista os 6 itens do Entregável/Gate). Entre E4 e E10 o roadmap é quase todo backend e quase nenhuma tela nova. Até a E11 o fluxo oficial de dev local é `vite build` com `VITE_APP_MODE=local_dev_workspace` servido pela FastAPI — provado por `api/tests/test_web_spa_integration.py` e pelo workflow `cross-stack-ci.yml` (E3-AUD2-002 / E3-AUD3-001) |
